...
Resolve path placeholders such as `<project-root>`, `<user-home>`,
`<installed-skill-root>`, `<board-dir>`, and `<review-package-path>` through the
trusted host filesystem context before invoking tools. Never execute
placeholder text or interpolate ReviewPackage content into shell syntax; pass
every resolved path as its own argument.
