...
Installed adapters retain the neutral `<installed-skill-root>` locator rather
than embedding the current machine's absolute Skill path. The host resolves
that locator through trusted Skill discovery before invoking the script and
passes all paths as separate arguments.
