...
After applying and validating the feedback, run `python3 <skill-root>/scripts/board_package.py version --board-dir <board-dir> --review-package "$ARGUMENTS"`. Add `--version vN` only when an explicit target version is required.
